...
def send_email(newfile):
    ...
    msg['From'] = '15651966757@163.com <XXX@163.com>'
...
if __name__=="__main__":
    ...
    test_dir = 'D:\\hanxing\\Test\Test\\testShopper\\shopper'
    # 知道测试报告的路径
    test_report_dir = 'D:\\hanxing\\Test\Test\\testShopper\\report'

    discover = unittest.defaultTestLoader.discover(test_dir, pattern='api_main_tests.py')
    ...
    # suite = unittest.makeSuite(core_server_tests)
    # now=time.strftime("%Y-%m-%d-%H_%M_%S",time.localtime(time.time()))
    # filename='D:\\hanxing\\Test\Test\\testShopper\\report\\report'+now+"test_all.html"
    # fp=file(filename,'wb')
    # runner=HTMLTestRunner.HTMLTestRunner(stream=fp,title=u'商家端接口测试报告',description=u'测试用例结果')
    # runner.run(suite)
    # fp.close()